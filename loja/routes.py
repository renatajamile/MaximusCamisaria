...
from flask import render_template, url_for, redirect, request, flash  # type: ignore
from loja import app, database, bcrypt, allowed_file
from loja.models import Usuario, Personalizada, Avaliacao
...
def criarconta():
    ...
    if form_criarconta.validate_on_submit():
        ...
        if Usuario.query.filter_by(email=form_criarconta.email.data).first():
            flash(
                'O e-mail informado já está cadastrado. Por favor, tente outro e-mail.', 'danger')
            return redirect(url_for('criarconta'))

        # Verificar se o CPF já existe
        if Usuario.query.filter_by(cpf=form_criarconta.cpf.data).first():
            flash(
                'O CPF informado já está cadastrado. Por favor, tente outro CPF.', 'danger')
            return redirect(url_for('criarconta'))

        senha = bcrypt.generate_password_hash(
            form_criarconta.senha.data).decode('utf-8')
        usuario = Usuario(
        ...
        try:
            ...
            flash(
                'Ocorreu um erro ao criar a conta. Por favor, tente novamente.', 'danger')
...
def editar_usuario(id_usuario):
    ...
    if usuario != current_user:
        abort(403)  # type: ignore # Proibir acesso se o usuário não for o atual
    ...
    if form.validate_on_submit():
        if form.foto.data:  # Verifica se foi enviado um arquivo de foto
            ...
            if file and allowed_file(file.filename):
                ...
                unique_filename = f"{uuid.uuid4().hex}_{
        ...
        cor_selecionada = form.cor.data if form.cor.data else ''
        ...
        if form.foto_perfil.data:
            ...
            if file and allowed_file(file.filename):
                ...
                if current_user.foto_perfil != 'default_profile.png':
                    try:
                        os.remove(os.path.join(
                            app.config['PROFILE_PIC_UPLOAD_FOLDER'], current_user.foto_perfil))
                ...
                unique_filename = f"{uuid.uuid4().hex}_{
                    datetime.now().strftime('%Y%m%d%H%M%S')}.{file_ext}"
                upload_path = os.path.join(
                    app.config['PROFILE_PIC_UPLOAD_FOLDER'], unique_filename)
                file.save(upload_path)
...
def carrinho():
    ...
    itens_personalizados = Personalizada.query.filter_by(
...
def deletar_item(id):
    ...
    if item.id_usuario != current_user.id:
        # type: ignore # Proibir acesso se o item não pertencer ao usuário atual
        abort(403)
